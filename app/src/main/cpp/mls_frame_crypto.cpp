...
Java_com_arpit_utmesh_messaging_call_sfu_MlsFrameCrypto_nativeCreateEncryptor(JNIEnv*, jclass) {
  resolveRust();
  return reinterpret_cast<jlong>(static_cast<shim::FrameEncryptorInterface*>(new MlsEncryptor()));
}

JNIEXPORT jlong JNICALL
Java_com_arpit_utmesh_messaging_call_sfu_MlsFrameCrypto_nativeCreateDecryptor(JNIEnv*, jclass) {
  resolveRust();
