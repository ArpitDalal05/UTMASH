package com.arpit.utmesh.security
 ...
 *
 * The signing identity is stored locally using AttestedKeyStore. Cached certificates are used when available.
 */
...
/**
    ...
    suspend fun ensureAuthorityMeshGroupKey(): Boolean = withContext(Dispatchers.IO) {
        AuthorityMeshKeyStore.loadGroupKey(context) != null
    }
    ...
    /** Loads a locally cached certificate when available. */
    suspend fun getOrFetchCertificate(
    deviceId: String = LocalKeyStorage.getOrCreateRescueDeviceId(context.applicationContext)
): Pair<java.security.KeyPair, ByteArray> = withContext(Dispatchers.IO) {

    val currentUid = resolveCurrentUid()

    require(currentUid.isNotEmpty()) {
        "A local UT identity is required."
    }

    val keyPair = AttestedKeyStore.loadExistingAttestedKey()
        ?: throw IllegalStateException(
            "No local attested signing key is available."
        )
    ...
    if (cachedCertificate != null) {
        Log.i(TAG, "Using cached local rescue certificate")
        return@withContext keyPair to cachedCertificate
    }

    throw IllegalStateException(
        "Online certificate provisioning is unavailable in offline UT Mesh mode."
    )
}
suspend fun provisionAndStoreCertificate(deviceId: String): RoleCertificate =
     ...
     */
    /**
     * Background certificate provisioning is disabled in offline UT Mesh mode.
     */
    fun warmUpCertificateInBackground() {
        Log.i(
            TAG,
            "Background certificate provisioning disabled in offline UT Mesh mode."
        )
    }


    /**
     * Server certificate revalidation is disabled in offline UT Mesh mode.
     ...
     */
    suspend fun revalidateAgainstServer(): String? =
    withContext(Dispatchers.IO) {
        Log.i(
            TAG,
            "Certificate server revalidation disabled in offline UT Mesh mode."
        )
        null
    }
    ...
    private fun resolveCurrentUid(): String {
        return LocalKeyStorage.getSavedUid(context)
            ?.trim()
            .orEmpty()
    }
