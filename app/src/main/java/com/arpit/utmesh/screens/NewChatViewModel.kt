        ...
        manualConnectJob = viewModelScope.launch(exceptionHandler) {
            ...
            if (!isValidMac(mac)) {
                ...
                _manualConnectState.value = ManualConnectState.Error(
                    context.getString(com.arpit.utmesh.R.string.manual_connect_error_mac)
                )
                return@launch
            }
            if (sessionCode.isEmpty()) {
                ...
                _manualConnectState.value = ManualConnectState.Error(
                    context.getString(com.arpit.utmesh.R.string.manual_connect_error_session)
                )
                return@launch
            }
                ...
                is DeviceResult.Error -> {
                    ...
                    val errorMessage = when (result.code) {
                        DeviceError.MISSING_PERMISSIONS -> context.getString(
                            com.arpit.utmesh.R.string.manual_connect_error_permissions
                        )

                        DeviceError.DEVICE_NOT_FOUND, DeviceError.PAIRING_FAILED -> context.getString(
                            com.arpit.utmesh.R.string.manual_connect_error_pairing
                        )
                        ...
                        DeviceError.BLUETOOTH_OFF -> context.getString(
                            com.arpit.utmesh.R.string.qr_scan_error_bluetooth_off
                        )
                    }
