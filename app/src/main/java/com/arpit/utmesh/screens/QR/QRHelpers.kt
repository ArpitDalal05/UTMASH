package com.arpit.utmesh.screens.QR
...
import com.arpit.utmesh.data.DeviceError
import com.arpit.utmesh.data.DeviceResult
import com.arpit.utmesh.data.deleteContact
import com.arpit.utmesh.data.getContact
import com.arpit.utmesh.data.getContactByRemoteSessionCode
import com.arpit.utmesh.data.getContactByPeerUid
import com.arpit.utmesh.data.getContacts
import com.arpit.utmesh.data.markContactVerified
import com.arpit.utmesh.data.normalizeMacAddress
import com.arpit.utmesh.data.normalizeVerifiedIdentityKey
import com.arpit.utmesh.data.pairDevice
...
import com.arpit.utmesh.service.p2p.resolveP2pShare
...
private fun isPlaceholderQrDisplayName(name: String): Boolean {
    ...
    return trimmed.isBlank() || trimmed.equals("UT Mesh", ignoreCase = true)
}
...
private fun announceOurIdentity(context: Context, savedSessionCode: String) {
    ...
    announceScope.launch {
        runCatching {
            ...
            val mySessionCode = com.arpit.utmesh.data.database.LocalKeyStorage
                .getOrCreateP2pSessionCode(appContext)
            val myName = runCatching {
                com.arpit.utmesh.getSavedUserName(appContext).first().trim()
            }.getOrNull()?.takeIf { it.isNotBlank() }
            val sent = com.arpit.utmesh.messaging.InternetChatTransport(appContext)
